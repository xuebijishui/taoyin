...
def get_img(img_url, i, run_time):
    ...
    try:
        ...
        print('发生错误：%s\n自动记录并跳过此图片' % e)
        global failed_img_urls_dict
        failed_img_urls_dict[str(i)] = img_url
        global success_img_amount
...
def get_and_save_imgs_once(run_time):
    
    global failed_img_urls_dict     
    global img_urls_dict
    # 如果你只是在开始声明了全局变量，但在函数中使用的时候没有声明，系统默认这个变量为局部变量。
    if run_time != 0:
        img_urls_dict = failed_img_urls_dict
    failed_img_urls_dict = {}
    ...
    if failed_img_urls_dict:
        print('%d张图片下载失败：' % len(failed_img_urls_dict))
        for i in failed_img_urls_dict:
            print('图%d：' % (int(i)+1), failed_img_urls_dict[i])
        print()
...
def crawler_for_single_url(url):
    ...
    if get_html(url):
        if no_prompt_message():
            ...
            print('共有目标图片%d张，成功下载%d张图片,失败%d张'% (img_amount, success_img_amount, img_amount-success_img_amount))
            print('图片保存至%s' % path)
            end = time.perf_counter()
            print('任务执行共%.2f秒' % (end-start))


def global_just_for_import_another():
    ...
    global img_amount
    img_amount = -1
    '''crawler_for_multiple_urls.py中根据img_amount的值是-1还是0,或者是正数来判断：
        若为-1，说明没有运行到get_img_urls()，在此之前就结束了
        （get_img_urls()中有img_amount = len(img_urls_dict)）
        若为0，说明该网页中没有图片网址
        若为正数，说明有正数张图片
    '''
...
if __name__ == '__main__':
    ...
    global_just_for_import_another()
    
    url = input('请输入要爬取图片的网址：\n')
    crawler_for_single_url(url)
