        ...
        if not html_source_code is None:
            ...
            if crawler.no_alert_error(html_source_code):
                ...
                if not html_title is None:
                    ...
                    dir_path = crawler.make_dir()    #参数为self.sub_path,无需再传入html_title
                        
                    crawler.save_html_source_code(dir_path, html_title, html_source_code)
                    
                    html_type = crawler.get_html_type(html_source_code)
                        
                    new_html_source_code = crawler.make_new_html(html_source_code, html_type, html_title)
                    
                    if not new_html_source_code is None:
                        ...
                        if img_list != []:
                            ...
                            for run_time in range(1,11):    #每张图片最多请求十次    
