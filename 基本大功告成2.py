...
def get_img_urls():
    ...
    '''
    re_str = r'((?<=onclick="zoom\(this, this\.src, 0, 0, 0\)" class="zoom" file=").+?(?=" onmouseover="img_onmouseoverfunc\(this\)" lazyloadthumb="1" border="0" alt="" />\s?<br />))|((?<=src="static/image/common/none.gif" zoomfile=").+?(?=" file="))'
...
def save_img(img_url, i):
    ...
    global img_name
    re_str = r'((?<=\d{4}/\d{2}/\d{2}/)|(?<=\d{6}/\d{2}/)).+?\.((png)|(jpg)|(gif)|(bmp)|(jpeg)|(webp))'
    img_name = re.search(re_str, img_url, flags=re.I).group()
